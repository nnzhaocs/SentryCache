...
NS=sc-eval-hotel
TRIAL_RUN=${SENTRYCACHE_ROOT}/eval/exp_5_2_hotel/run_trial.sh
...
start_controller() {
  ...
  L2_HOST="$L2_IP" SC_NAMESPACE=$NS \
    SC_LABEL_KEY=sc-deployment SC_LABEL_VALUE=profile \
    CACHE_BACKEND=memcached SIDECAR_PORT=11211 \
    PREFETCH_TOPN=200 PYTHONPATH=${SENTRYCACHE_ROOT}/controller \
    nohup python3 "$CONTROLLER" $extra > /tmp/controller_hr_${mode}.log 2>&1 &
  sleep 4
